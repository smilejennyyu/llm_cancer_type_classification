...
def load_and_prepare_data():
    ...
    o3_mini_path = "/data1/morrisq/yuj13/llm_genomics/genie_output/o3-mini/filtered_batch_msk"
    ...
    o3_mini_path_non_msk = "/data1/morrisq/yuj13/llm_genomics/genie_output/o3-mini/filtered_batch_non_msk"
    ...
    # Process o3-mini data with normalization
    ...
    joint_pred = joint_pred.rename(columns={
        'prediction1': 'o3_mini_Prediction1', 'prediction2': 'o3_mini_Prediction2',
        'prob1': 'o3_mini_Conf1', 'prob2': 'o3_mini_Conf2',
    ...
    cols_to_check = ['o3_mini_Prediction1', 'GDD_Prediction1', 'ground_truth', 'CENTER']
    ...
    print(f"Unique o3-mini predictions: {joint_pred['o3_mini_Prediction1'].nunique()}")
...
def prepare_features_and_labels(joint_pred):
    ...
    features_all = joint_pred[[
        'o3_mini_Prediction1', 'o3_mini_Conf1',
        'GDD_Prediction1', 'GDD_Conf1',
        'o3_mini_Prediction2', 'o3_mini_Conf2',
    ...
    features_encoded_all = pd.get_dummies(features_all, columns=[
        'o3_mini_Prediction1', 'GDD_Prediction1', 'o3_mini_Prediction2', 'GDD_Prediction2',
        'o3_mini_Conf1', 'GDD_Conf1', 'o3_mini_Conf2', 'GDD_Conf2'
    ])
    ...
    # Handle o3-mini predictions with potential unseen classes
    o3_mini_preds = joint_pred['o3_mini_Prediction1'].apply(
        lambda x: label_encoder.transform([x])[0] if x in label_encoder.classes_ else len(label_encoder.classes_)
    )
    original_preds_encoded_llm = o3_mini_preds.values
    
    return features_encoded_all, labels_encoded, original_preds_encoded, original_preds_encoded_llm, label_encoder

def run_5_fold_cv(features, labels, original_gdd, original_o3mini, output_dir):
    ...
    for i, (train_idx, test_idx) in enumerate(skf.split(features, labels)):
        ...
        acc_gdd = accuracy_score(y_test, original_gdd[test_idx])
        acc_o3 = accuracy_score(y_test, original_o3mini[test_idx])
        ...
        results['o3_mini_accuracy'].append(acc_o3)
    ...
    print(f"5-fold CV detailed results saved to: {cv_detailed_file}")
    
    # Calculate and save summary statistics
    methods = ['GDD_accuracy', 'o3_mini_accuracy', 'LogReg_accuracy', 'XGBoost_accuracy', 'RandomForest_accuracy']
    method_names = ['GDD Original', 'o3-mini Original', 'LogReg', 'XGBoost', 'Random Forest']
    
    summary_data = {
        'Method': method_names,
        'Mean_Accuracy': [results_df[method].mean() for method in methods],
        'Std_Accuracy': [results_df[method].std() for method in methods],
        'Min_Accuracy': [results_df[method].min() for method in methods],
        'Max_Accuracy': [results_df[method].max() for method in methods]
    }
    ...
    for _, row in summary_df.iterrows():
        print(f"{row['Method']}: {row['Mean_Accuracy']:.4f} ± {row['Std_Accuracy']:.4f}")
    
    return results_df, summary_df

def run_leave_one_center_out(joint_pred, features, labels, original_gdd, original_o3mini, label_encoder, output_dir):
    ...
    for center in unique_centers:
        ...
        original_preds_o3_test = original_o3mini[test_mask]
        ...
        # Calculate accuracies for original models
        acc_gdd = accuracy_score(y_test, original_preds_gdd_test)
        acc_o3 = accuracy_score(y_test, original_preds_o3_test)
        
        # Initialize accuracy variables
        acc_log = acc_xgb = acc_rf = 0.0
        ...
        try:
            ...
            if len(y_test_relabeled) > 0:
                ...
                # Calculate accuracy only for valid test samples
        ...
        print(f"o3-mini accuracy: {acc_o3:.4f}")
    ...
    summary_data = {
        'Method': ['GDD Original', 'o3-mini Original', 'LogReg', 'XGBoost', 'Random Forest'],
        'Mean_Accuracy': [],
        'Std_Accuracy': [],
        'Min_Accuracy': [],
        'Max_Accuracy': [],
    ...
    for method in ['GDD_accuracy', 'o3_mini_accuracy', 'LogReg_accuracy', 'XGBoost_accuracy', 'RandomForest_accuracy']:
        mean_acc, std_acc, min_acc, max_acc = safe_stats(results_df[method])
        successful_runs = len(results_df[results_df[method] > 0.0]) if method not in ['GDD_accuracy', 'o3_mini_accuracy'] else len(results_df)
        
        summary_data['Mean_Accuracy'].append(mean_acc)
        summary_data['Std_Accuracy'].append(std_acc)
        summary_data['Min_Accuracy'].append(min_acc)
        summary_data['Max_Accuracy'].append(max_acc)
    ...
    methods = ['GDD_accuracy', 'o3_mini_accuracy', 'LogReg_accuracy', 'XGBoost_accuracy', 'RandomForest_accuracy']
    method_names = ['GDD Original', 'o3-mini Original', 'Logistic Regression', 'XGBoost', 'Random Forest']
    ...
    for _, row in summary_df.iterrows():
        if row['Successful_Runs'] > 0:
            print(f"{row['Method']}: {row['Mean_Accuracy']:.4f} ± {row['Std_Accuracy']:.4f} ({row['Successful_Runs']} successful runs)")
        else:
...
def main():
    ...
    features, labels, original_gdd, original_o3mini, label_encoder = prepare_features_and_labels(joint_pred)
    ...
    cv_results, cv_summary = run_5_fold_cv(features, labels, original_gdd, original_o3mini, output_dir)
    
    # Run leave-one-center-out cross-validation
    loco_results, loco_summary = run_leave_one_center_out(
        joint_pred, features, labels, original_gdd, original_o3mini, label_encoder, output_dir
    )
    
    # Save overall summary
    overall_summary = {
        'Evaluation_Method': ['5-Fold CV', 'Leave-One-Center-Out'],
        'Best_Method': [
